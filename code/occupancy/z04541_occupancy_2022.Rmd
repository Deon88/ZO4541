...
Human distance is calculated lazily, just using a function to generate declining trends. In reality I could have used a linear equation, but meh.
...
for(s in 1:n_site){
  for(r in 1:n_region) {
    mean_d <- runif(1, 8, 75)
    human[, s, r] <- generate.trend(nyears = n_year, 
                                    mu1 = mean_d, 
                                    change = runif(1, max = ifelse(mean_d < 35, -1, -10), min = ifelse(mean_d < 35, -10, -35)))$mu
  }
}
#}
...
for(t in 2:n_year) {
  ...
  colonise_p[t, , ] <- plogis(-1 + -1 * snake[t, , ] + 2 * rabbit[t, , ])

  # Probability of extinction
  extinct_p[t, , ] <- plogis(-2 + 4.75 * buster[t, , ] - 0.03 * forest[t, , ] - 0.05 * human[t, , ])
