...
* Therefore, because we do know how many animals we caught, the objective to estimate the number of animals we did not catch.
  ...
  + _Number of captured animals + number of uncaptured animals = total population size._
...
* The number of animals that we never caught is an estimated parameter (where a parameter is an unknown single value, e.g., 18, which is estimated from the data we have observed).
  ...
  + In `MARK`, this parameter is referred to as `f0`.


* To estimate `f0` (number of animals never caught), we need to estimate what the capture probabilities are

  + This means that we assume _imperfect detection_, i.e., we assume we have not caught all animals in a population.


* Estimating capture probabilities is how we figure out how many animals were not caught.

  + If we know we caught 10 animals with a capture rate of 50%, then it is very likely there were 10 animals we never caught.
  
  + Note that this becomes more complex when we trap over multiple days.
